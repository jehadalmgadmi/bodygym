...
class _MyWidgetState extends State<HomeGym> {
 SqlDb sql1=SqlDb();
          ...
          children: [
            Expanded(
                ...
                child: Image(
                    width: 100,
                    height: 130,
                    image: AssetImage(exsrsis_list[index].main_image))),
  ...
  SELECT * FROM exrsise WHERE exsrsis_name = '${exsrsis_list[index].name}'
''');
...
if (result.isEmpty) {
  ...
  int response = await sql1.insertData('''
    ...
    VALUES (
      ...
      '${exsrsis_list[index].image1}',
      '${exsrsis_list[index].image2}',
      ${exsrsis_list[index].explane_state ? 1 : 0}
    )
  ''');
             setState(() {
                
               exe.add_exrsise(exsrsis_list[index]); 
